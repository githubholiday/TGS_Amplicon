...
def get_dic(file1):
	'''
	dic:key-s-开头的物种名，value-reads数
	spec:key-s-开头的物种名，value-物种全长名
	'''
	dic = {}
	spec = {}
...
def main():
	...
	for i in args.input:
		...
		all_dic[file_name],spec_dic = get_dic(i) #all_dic[file_name]：物种和read对应关系；spec_dic：物种和全称对应关系
		all_spec.update(spec_dic)
		species.extend(list(all_dic[file_name].keys())) #物种名
	...
	if args.rich:
		...
		#rich_dic[file_name] = get_count(all_dic,all_spec)
		#uniq_rich_species = set(rich_species)
		#common_dic(all_dic,uniq_rich_species,args.rich,sample)
