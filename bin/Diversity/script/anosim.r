...
dev.off()
out_data[1,] = c("all",anosim$statistic, anosim$signif)

combine = combn(group_name,2)
for (i in 1:ncol(combine)){
    samples = group[which(group$Group == combine[1,i] | group$Group == combine[2,i]),]
    samples
    a_tmp = a[rownames(a)  %in%  samples$Sample,]
    b_tmp = data.frame(a_tmp)
    b_tmp$Sample = rownames(a_tmp)
    group_tmp = merge(b_tmp,samples,sort=F)[,"Group"]
    dim(a_tmp)
    dim(group_tmp)
    anosim_tmp = anosim(a_tmp, group_tmp, permutations=999)
    par(mar = c(5,5,5,5))
    result_tmp = paste("R=",anosim$statistic,"    p=", anosim$signif)
    ...
    dev.off()
    out_data[i+1,] = c(paste(combine[1,i],"-",combine[2,i],sep=''),anosim$statistic, anosim$signif)
}
write.table(out_data,file=paste(outdir,"/anosim.stat.xls",sep=''),row.names=F)
