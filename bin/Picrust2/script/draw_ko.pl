...
cog <- cog[-1,]
cog.des <- cog[,1]
str_len = length(cog.des)
label = seq(0,str_len)
cog.des <- gsub(" \$","",cog.des)

cog.des <- paste(as.character(label),cog.des,sep=": ")
cog.num <- as.numeric(as.matrix(cog[,2]))
...
for(i in length(cog.des):1){
    text(0.01,i+15,cog.des[length(cog.des)+1-i],pos=4,cex=0.7)
}
par(op1)
par(def.par)
